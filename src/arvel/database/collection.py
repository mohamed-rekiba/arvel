"""arvel.database.collection — ``ModelCollection[M]``: the model-aware result set returned by
...
**Divergence:** a raw (non-hydrating) table-builder ``get()`` — no model bound —
still returns a plain ``list[dict]``; only *hydrated* results become an ``ModelCollection``.
Transformations that build a NEW collection from arbitrary values (``map``/``filter``/…, inherited
from the base ``Collection``) return a plain ``Collection``, not ``ModelCollection`` — the
...
class ModelCollection[M](Collection[M], _Sequence[M]):
    """A ``Collection`` of hydrated models, with model-aware batch operations. ``M`` is
    ...
    def __getitem__(self, index: Any) -> Any:
        if isinstance(index, slice):
            return ModelCollection(self._items[index])
    ...
    def contains(self, item: Any) -> bool:
        """Whether ``item`` — a member (by primary key) OR a bare key — is present (a key-aware variant of the base ``Collection.contains``)."""
        if self._items and isinstance(item, type(self._items[0])):
            return self.find(self._pk(item)) is not None
        return self.find(item) is not None

    async def load(self, *relations: str) -> ModelCollection[M]:
    ...
    async def load_missing(self, *relations: str) -> ModelCollection[M]:
    ...
    async def fresh(self) -> ModelCollection[M]:
        ...
        return ModelCollection([by_key[key] for key in self.model_keys() if key in by_key])

    def make_hidden(self, *keys: str) -> ModelCollection[M]:
        """Hide ``keys`` on every member's serialization (fans ``Model.make_hidden``)."""
        for m in self._items:
            cast("Any", m).make_hidden(*keys)
        return self

    def make_visible(self, *keys: str) -> ModelCollection[M]:
    ...
    def only(self, keys: Iterable[Any]) -> ModelCollection[M]:
        """Members whose primary key is in ``keys`` (the model-aware ``only`` —
        keyed by primary key, unlike the base ``Collection`` which has no such method)."""
        allowed = list(keys)
        return ModelCollection([m for m in self._items if self._pk(m) in allowed])

    def except_(self, keys: Iterable[Any]) -> ModelCollection[M]:
