...
def _cli_str(value: object) -> str:
    """A string option's value — or ``""`` when a command function was called directly (not via
    the CLI), where typer leaves its truthy ``OptionInfo`` sentinel in place of the default."""
    return value if isinstance(value, str) else ""


def _render_stub(template: str, **tokens: str) -> str:
    """Substitute the documented ``{token}`` placeholders — and nothing else. A stub is Python
    source, full of legitimate braces (dict/set literals, f-strings, ``{}`` in docstrings), so
    whole-template ``str.format`` would blow up on any user-edited stub; only literal token
...
def generate_migration(
    ...
    if create_table:
        ...
        body = _render_stub(template, cls=cls, table=create_table)
    else:
        template = _load_stub(base, "migration.generic.stub", _GENERIC_MIGRATION)
        body = _render_stub(template, cls=cls)
...
def generate_test(name: str, base: Path | None = None, *, force: bool = False) -> Path:
    ...
    target.write_text(_render_stub(template, name=slug))
    return target
...
def generate(kind: str, name: str, base: Path | None = None, *, force: bool = False) -> Path:
    ...
    if target.exists() and not force:
        ...
        raise FileExistsError(message)
    target.write_text(_render_stub(template, name=name))
    return target
...
def make_migration(
    ...
    # direct (non-CLI) calls leave typer's OptionInfo defaults in place — a truthy sentinel that
    # str.format used to silently render into the file; normalize to "not given" instead
    create = _cli_str(create)
    table = _cli_str(table)
    try:
