...
that single action. Give the class an `__call__` method and bind the **class** directly to a route —
no `Controller` base, no action name:
...
Pass the class, not an instance — the kernel instantiates it via the container per request (the same
`app.make(...)` path middleware classes go through), so constructor dependencies resolve normally.
...
(`PostController.authorize_resource(Post)` after the class body is the equivalent imperative form.)
Each action maps to an ability, checked against either the model class or the route-bound instance:
...
Instance actions authorize against the route-bound model (pair with [route–model binding](routing.md));
a denied check raises `403` before the action body runs.
