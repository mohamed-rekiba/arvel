...
You'll also need a `media` table once, in a migration — `Media.__table__` describes its columns
(the polymorphic owner `model_type`/`model_id`, `collection_name`, `file_name`, `disk`, `size`, the
JSON `custom_properties` and `generated_conversions`, and `order_column`).
