# ADR-032: Write-path completeness (insert_or_ignore / upsert count / truncate / insert_using / increment_each)

Status: Accepted (delivered WI-arvel-011)

Eloquent-parity increment (backlog `005`, story S8). No HTTP or schema surface —
recorded as an ADR. Builds on the WHERE-predicate engine (ADR-030).

## ADR-032-01: `upsert` issues one multi-row statement and returns a count
...
## ADR-032-02: `insert_or_ignore` is dialect-routed, not emulated
...
## ADR-032-03: `truncate` is a hard wipe, dialect-aware
...
## ADR-032-04: `insert_using` reuses the source builder's compiled SELECT
...
## ADR-032-05: `increment_each` / `decrement_each` bump many columns in one UPDATE
