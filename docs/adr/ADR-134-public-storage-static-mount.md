...
- Two coexisting local-serving modes: the `serve=true` app route (ADR-080, at `STORAGE_LOCAL_URL`, signed temp URLs) and this static mount (at `/storage`). They collide only if `STORAGE_LOCAL_URL` is also `/storage`. To point `Storage.url()` at the static path, set `STORAGE_LOCAL_URL=/storage`.
