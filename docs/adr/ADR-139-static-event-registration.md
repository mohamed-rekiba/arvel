# ADR-139: Static event registration + custom event objects
...
## ADR-139-01: `Model.on(event, callback)` wraps the observer machinery
...
## ADR-139-02: `__dispatches_events__` maps lifecycle names to bus events
...
## ADR-139-03: `__observed_by__` auto-registers at class-definition time
...
## ADR-139-04: `ModelEvent` is defined directly, not via a factory
