# ADR-133: Subquery FROM / JOIN / SELECT

Status: Accepted (delivered WI-arvel-012)

Eloquent-parity increment (backlog `005`, story S3). No HTTP or schema surface —
recorded as an ADR. Builds on the WHERE-predicate engine (ADR-130) and reuses the
existing row-shaping markers in `all()`.

## ADR-133-01: `on` is a callable that receives the aliased subquery
...
## ADR-133-02: `from_sub` returns dicts, not model instances
...
## ADR-133-03: `select_sub` / `add_select` append via the `__with_agg__` path
