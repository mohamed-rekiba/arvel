# ADR-118: Framework Query Builder Critical Fixes

See SAD-043 for full context. This ADR records the two non-obvious decisions.

## ADR-043-01: Route `Model.find()` through the query builder
...
## ADR-043-02: Raise `ValueError` on unknown `where_any()` operators
