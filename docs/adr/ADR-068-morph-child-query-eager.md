# ADR-068: MorphOne/MorphMany query + eager integration
...
## ADR-068-01: `morph_child` relation kind
...
so the owner token honours the morph map (ADR-066).

## ADR-068-02: Existence + count subqueries
...
## ADR-068-03: Batched eager loading
...
## ADR-068-04: `Model.load()` routes async relations
