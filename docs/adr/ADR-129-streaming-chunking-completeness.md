# ADR-129: Streaming and Chunking Completeness
...
## ADR-129-01: `stream()` is a true server-side cursor, distinct from `lazy()`
...
## ADR-129-02: Directional keyset — `descending=` on `chunk_by_id`, plus `lazy_by_id`
...
## ADR-129-03: Callbacks can stop early by returning `False`
...
## ADR-129-04: Offset `chunk` enforces an order by primary key
