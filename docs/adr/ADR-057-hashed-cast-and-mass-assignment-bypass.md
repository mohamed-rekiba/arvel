# ADR-057: `hashed` Cast and Explicit Mass-Assignment Bypass
...
## ADR-057-01: `hashed` is a write-only cast over the existing `__casts__` dispatch
...
## ADR-057-02: `force_fill` bypasses guards; `unguarded()` is a scoped context only
