# ADR-157 — Clean model syntax: type-inferred columns + `field()`

**Status**: Accepted (delivered)
**Date**: 2026-06-01
**Supersedes**: —
**Related**: ADR-046 (Model metaclass forwarding), ADR-076 (`MappedAsDataclass` for typed `__init__`), ADR-108 (`Model.id` VARCHAR)

## Context

ADR-076 made `Model` construction typed and removed the `Mapped[...]` wrapper from declarations, but a column still needed an explicit helper on every field:
...
SQLAlchemy 2.0 already infers a column's SQL type from a `Mapped[T]` annotation via `registry.type_annotation_map` plus a bare `mapped_column()`. `ModelMeta` (ADR-046) already rewrites plain annotations to `Mapped[...]`. The question was whether to lean on that or build custom inference.
