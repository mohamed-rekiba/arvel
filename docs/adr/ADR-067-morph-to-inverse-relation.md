# ADR-067: MorphTo inverse relation

Status: Accepted (delivered WI-arvel-027)

Epic 007 Story 2. Builds on the morph map (ADR-066). Adds the child→parent side of polymorphism —
`comment.commentable`.

## ADR-067-01: `MorphTo` descriptor + accessor
...
## ADR-067-02: `associate` / `dissociate`
...
## ADR-067-03: Batched eager loading grouped by type
...
## ADR-067-04: Why store the morph name (not the descriptor) on `_RelationTarget`
