# ADR-038: Date/time, LIKE, and join helpers

Status: Accepted

Eloquent-parity increment (backlog `005`, stories S1, S5, S6). All build on the WHERE
predicate engine from ADR-030.

## ADR-038-01: Date/time WHERE helpers via `extract`
...
## ADR-038-02: LIKE + multi-column helpers
