...
## ADR-140-01: Hook attachment moves to `Model.__init_subclass__`
...
## ADR-140-02: `CREATED_AT` / `UPDATED_AT` constants + `__timestamps__` toggle
...
## ADR-140-03: `without_timestamps()` is a task-local async context
...
## ADR-140-04: `touch(attribute=None)` saves through the event path
