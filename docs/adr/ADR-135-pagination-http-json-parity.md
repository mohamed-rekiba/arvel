# ADR-135: Pagination HTTP + JSON parity
...
## ADR-135-01: Request resolution via a contextvar, not an HTTP import in the DB layer
...
## ADR-135-02: Two serialization shapes — `to_dict` (nested) and `to_response` (flat)
...
## ADR-135-03: `appends` / `with_query_string` / `fragment` are immutable chainables
...
## ADR-135-04: Bidirectional cursors via a direction flag in the token
