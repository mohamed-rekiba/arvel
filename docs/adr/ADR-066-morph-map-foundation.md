# ADR-066: Morph map foundation

Status: Accepted (delivered WI-arvel-025+1 / WI-arvel-026)

Supersedes the unqualified-class-name default of ADR-066 for the polymorphic `{name}_type` token.
First story of Epic 007 (relationship parity).

## Context

ADR-066 stored the owner's *unqualified class name* (`"Post"`) in the morph discriminator column.
...
## ADR-066-01: Process-global morph map
...
## ADR-066-02: Token resolution, both directions

Status: Accepted

- `get_morph_alias(cls)` (write side): the mapped alias if `cls` is in the map, else the short class
  name (the ADR-066 default — so unmapped apps behave exactly as before).
...
## Merged: Morph Discriminator: Short Class Name (was ADR-066)
