# ADR-009: Enforce Zero-Warning Quality Gates
...
[ADR-008](ADR-008-mypy-pyright-parity.md) established that `mypy --strict` and `pyright --strict` must both pass. That rule was about which checkers run. It did not say what to do when pyright reports a `warning` instead of an `error`.
...
- **Pre-existing suppressions were grandfathered** under FB-010-001, which has since closed (see [ADR-010](ADR-010-two-checker-suppression-floor.md)): the cleanup pass reduced the floor and the remaining suppressions are categorized as the irreducible dual-checker minimum. New code must still not add unjustified suppressions.

## Consequences

- **Gain**: The strict-typing claim in the README, the docs index, and ADR-008 is now enforced end-to-end. A contributor can trust that `make pre-commit` clean implies a zero-warning baseline.
