# ADR-019 — `with_routing(...)` loader design
...
The canonical layout (ADR-018) puts route declarations in
...
**Enforcement**:
- Dedicated unit test in `packages/arvel/tests/application/test_loader.py`
  for the `sys.path` invariant (Gate #28).
- Integration test under `tests/feature/` of the smoke skeleton verifies
  that `routes/web.py` registers `GET /` and that the response is 200.
