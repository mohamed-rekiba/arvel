# ADR-010: Two-Checker Policy and the Irreducible Suppression Floor

**Status**: Accepted
**Date**: 2026-05-18
**Last reconciled**: 2026-06-01
**Supersedes**: nothing; complements [ADR-008](ADR-008-mypy-pyright-parity.md) and [ADR-009](ADR-009-enforce-quality-gates.md).

> **Reconciliation note (2026-06-01)**: the suppression counts, line totals,
> and file locations below are the point-in-time snapshot from the FB-010-001
> pass. The **policy** (specific codes only, seven accepted categories, no
> numeric budget) is current; the exact numbers drift with the codebase and are
> not re-measured here. Locations that named the removed `arvel-cli` package
> have been corrected to their current homes.

## Decision

Running `mypy --strict` **and** `pyright --strict` on the same code (per ADR-008) creates a small, irreducible set of lines where one checker requires a suppression that the other rejects. The framework accepts that floor as the cost of dual-checker enforcement. Specifically:
...
2. **Every suppression must be specific.** Bare `# type: ignore` and bare `# noqa` are forbidden ([ADR-009](ADR-009-enforce-quality-gates.md)). Every suppression must reference at least one error code and (where the cause is non-obvious) carry a one-line explanation.
...
ADR-008 mandates parity between `mypy --strict` and `pyright --strict`. ADR-009 mandates zero errors and zero warnings from every gate. Combined, the two policies forbid letting either checker complain.
...
**B. Drive to zero suppressions.** Possible only by removing one of the checkers (violates ADR-008) or by relaxing strictness (violates ADR-009) or by widening to `Any` everywhere ([forbidden by `.cursor/rules/enforce-quality-gates.mdc`](https://github.com/mohamed-rekiba/arvel/blob/main/.cursor/rules/enforce-quality-gates.mdc)). Rejected.
...
- [ADR-008 — Enforce both `mypy --strict` and `pyright --strict`](ADR-008-mypy-pyright-parity.md)
- [ADR-009 — Enforce zero-warning quality gates](ADR-009-enforce-quality-gates.md)
...
- FB-010-001 commit chain: `8795c86` (phase 1) → `d1425ab` (phase 7) plus the ADR-010 second pass (248 → 143 suppression-bearing lines, -42.3 %)
