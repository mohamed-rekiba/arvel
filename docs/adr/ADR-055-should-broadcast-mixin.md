# ADR-055: `ShouldBroadcast` is a Mixin on `Event`, Not a Separate Listener Type
...
Matches the existing `ShouldQueue` mixin pattern (WI-009 ADR-040), keeps the Laravel mental model 1:1, and composes cleanly with `ShouldQueue` (an event can be both queued AND broadcasted).
...
- Default `broadcast_with()` returns `model_dump()` for Pydantic-based events (which is all of them post-ADR-037). Users override only when they need to trim or shape the payload.
