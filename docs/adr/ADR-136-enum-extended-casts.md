# ADR-136: Enum and extended built-in casts
...
## ADR-136-01: An `Enum` subclass is a valid cast spec
...
## ADR-136-02: `object` and `collection` are read-path-only, like `array`
...
## ADR-136-03: `datetime:FORMAT` parses with a fallback and serializes with `strftime`
...
## ADR-136-04: serialize skips `None`
