# ADR-088 — Email validation at the API boundary, not on the column
...
5. **One concept, one place.** Validation rules live in the boundary schema. Persistence is dumb storage. The split is the same one the framework uses everywhere else (ADR-021 + the `PydanticType` doctrine).
...
| SQLModel | Rejected wholesale in ADR-022 / research 002 — Pyright unsupported, PEP 649 breakage, metaclass fusion. Not specific to email. |
