# ADR-066 — Per-message delay and priority as first-class `Job` fields

**Status**: Accepted
**Date**: 2026-05-19
**Supersedes**: none
**Superseded by**: none
**Related**: ADR-033 (Job is BaseModel), ADR-035 (Job registry allowlist), ADR-036 (Worker retry/DLQ)
...
| `taskiq` + `redis://` | `taskiq-redis` schedule source | queue-name suffix `:p<N>` (see ADR-067) |
