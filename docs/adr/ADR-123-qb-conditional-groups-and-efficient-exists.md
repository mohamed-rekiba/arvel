# ADR-123: Query Builder Conditional Groups, `unless`/`tap`, and Efficient `exists`
...
## ADR-123-01: Nested `WHERE` groups via a callback that returns a builder
...
## ADR-123-02: `unless` is `when` with a negated condition; `tap` is side-effect only
...
## ADR-123-03: `exists` issues `SELECT EXISTS(...)`, not `COUNT(*) > 0`
