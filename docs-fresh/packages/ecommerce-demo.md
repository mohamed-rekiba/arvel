...
**Source**: `packages/arvel-ecommerce-demo/` — `backend/` (Arvel app) and `frontend/` (Vue 3 + Vite SPA).

## What's in it

```
packages/arvel-ecommerce-demo/
├── backend/
...
> **Note**: The demo uses `arvel-permission`'s **models and traits** on its `User` (with an inlined RBAC migration) but does **not** register `PermissionServiceProvider`. So the Gate `before` hook and `apply_*_config` from the package don't run — RBAC checks go through the demo's own wiring. It also doesn't register `OAuthServiceProvider`, `SearchServiceProvider`, or `AuditServiceProvider`.

## Workspace status

The demo is picked up by the `packages/*` workspace glob, but unlike the five libraries it's **not** listed in the root `[tool.uv.sources]` or the dev dependency group. CI type-checks and tests target the five libraries; the demo is exercised through its own tests and `Makefile`.
