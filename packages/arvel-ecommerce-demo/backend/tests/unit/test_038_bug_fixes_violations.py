...
class TestForceDestroyUsesRoleLevel:
    ...
    def test_force_destroy_routes_require_level_100(self) -> None:
        ...
        src = (Path(__file__).parent.parent.parent / "routes" / "api.py").read_text()
        assert '_require_role_level(request, "products.delete", 100)' in src
        assert '_require_role_level(request, "categories.delete", 100)' in src
        assert '_require_role_level(request, "vendors.delete", 100)' in src
    ...
    def test_role_assignment_cannot_exceed_actor_level(self) -> None:
        ...
        src = (Path(__file__).parent.parent.parent / "routes" / "api.py").read_text()
        assert "role_level = await _role_level(payload.role)" in src
        assert "if not await actor.has_level(role_level)" in src
