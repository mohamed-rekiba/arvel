...
def test_frontend_has_runnable_vite_entrypoint() -> None:
    ...
    assert package_json["dependencies"]["vue"] == "3.5.34"
    assert package_json["dependencies"]["vue-router"] == "5.0.7"
...
def test_admin_user_detail_manages_roles_and_permissions() -> None:
    ...
    assert '@Route.get("/api/admin/users/{user_id}"' in routes
    assert '"/api/admin/users/{user_id}/force"' in routes
