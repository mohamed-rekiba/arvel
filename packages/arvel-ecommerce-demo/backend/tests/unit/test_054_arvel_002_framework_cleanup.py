...
ARVEL_AUTH_GUARDS = (
    Path(__file__).parents[5] / "packages" / "arvel" / "src" / "arvel" / "auth" / "guards.py"
)
...
def test_demo_models_use_uuid_id() -> None:
    """All UUID-PK models in the demo use uuid_id() not raw mapped_column."""
    model_files = [
...
def test_demo_base_re_exports_translatable_mixin() -> None:
    demo_base = BASE / "app" / "models" / "base.py"
    src = _src(demo_base)
...
def test_arvel_auth_guards_exists() -> None:
    assert ARVEL_AUTH_GUARDS.exists(), "arvel/auth/guards.py does not exist"
...
def test_arvel_auth_guards_lazy_permission_import() -> None:
    """arvel_permission must NOT be imported at module level in guards.py."""
    src = _src(ARVEL_AUTH_GUARDS)
...
def test_demo_deps_imports_guards_from_arvel() -> None:
...
def test_routes_api_wires_auth_controller() -> None:
    src = _src(ROUTES_API)
    # Auth routes wired to a controller that's either the framework's or the demo's subclass
