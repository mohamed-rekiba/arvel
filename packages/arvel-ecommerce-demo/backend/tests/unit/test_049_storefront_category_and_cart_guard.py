...
def test_storefront_category_route_uses_published_products_service() -> None:
    routes = _src(API_ROUTES)

    assert '@Route.get("/api/categories/{slug}"' in routes
    assert "list_published_by_category_slug(" in routes
    assert 'resolved_locale = locale or getattr(request.state, "locale", "en") or "en"' in routes
...
def test_cart_payloads_reject_non_positive_quantities() -> None:
    ...
    assert "quantity: Annotated[int, Field(ge=1)] = 1" in routes
    assert "quantity: Annotated[int, Field(ge=1)]" in routes
