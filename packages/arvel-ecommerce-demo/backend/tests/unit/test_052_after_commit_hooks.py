...
async def test_callbacks_execute_after_successful_transaction() -> None:
    ...
    # Force .begin to be a plain MagicMock so .begin() returns its return_value
    # (not a coroutine, which is what AsyncMock children produce in Python 3.10+).
    mock_session.begin = MagicMock()
    mock_session.begin.return_value.__aenter__.return_value = mock_session
    mock_session.begin.return_value.__aexit__.return_value = False

    mock_maker = MagicMock()
    mock_maker.return_value.__aenter__.return_value = mock_session
    mock_maker.return_value.__aexit__.return_value = False
