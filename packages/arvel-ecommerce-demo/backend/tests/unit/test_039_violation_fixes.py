...
class TestV002GuardName:
    ...
    def test_role_lookup_uses_api_guard(self) -> None:
        src = _routes_src()
        assert 'guard_name="api"' in src, (
            "V-002 not fixed: guard_name='api' not found in routes/api.py"
        )
...
class TestV008UserLifecycleEndpoints:
    ...
    def test_user_restore_route_exists(self) -> None:
        src = _routes_src()
        # There's already a product restore — check for user-specific restore
        assert "users_restore" in src or "users/{user_id}/restore" in src, (
            "V-008 not fixed: no user restore route in routes/api.py"
        )

    def test_suspend_requires_users_manage_permission(self) -> None:
        src = _routes_src()
        # The suspend route handler must call _require_permission with users.manage
        assert '"users.manage"' in src, "V-008 not fixed: users.manage permission check missing"
...
class TestV009RevokePermissionEndpoint:
    ...
    def test_revoke_permission_to_called(self) -> None:
        src = _routes_src()
        assert "revoke_permission_to" in src, (
            "V-009 not fixed: revoke_permission_to not called in routes/api.py"
        )

    def test_revoke_permission_requires_roles_manage(self) -> None:
        src = _routes_src()
        assert '"roles.manage"' in src, "V-009 not fixed: roles.manage permission check missing"
