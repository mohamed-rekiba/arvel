...
- published_products materialized view exists after migrations
- GIN indexes exist on products.name, categories.name, products.search_vector
- Unique index on published_products.id exists (required for REFRESH CONCURRENTLY)
...
@pytest.mark.asyncio
async def test_published_products_materialized_view_exists(fresh_db: str) -> None:
    """US-002: published_products materialized view exists after migrations."""
    from sqlalchemy import text
    from sqlalchemy.ext.asyncio import create_async_engine
    ...
    try:
        async with engine.connect() as conn:
            result = await conn.execute(
                text("SELECT matviewname FROM pg_matviews WHERE matviewname = 'published_products'")
            )
            row = result.fetchone()
            assert row is not None, "published_products materialized view not found"
    finally:
        await engine.dispose()


@pytest.mark.asyncio
async def test_published_products_has_unique_index(fresh_db: str) -> None:
    """US-002: unique index on published_products.id exists (REFRESH CONCURRENTLY requirement)."""
    from sqlalchemy import text
    from sqlalchemy.ext.asyncio import create_async_engine
        ...
        async with engine.connect() as conn:
            result = await conn.execute(
                text(
                    ...
                    "WHERE tablename = 'published_products' AND indexdef LIKE '%UNIQUE%'"
                )
            )
            row = result.fetchone()
            assert row is not None, (
                "No unique index on published_products — REFRESH CONCURRENTLY will fail"
            )
    finally:
        await engine.dispose()
