"""Storefront feature tests — US-018 (product listing) + US-019 (search).

RED: imports from app.bootstrap fail until Stage 3b. All tests are RED.

Acceptance criteria:
- US-018: GET /api/products returns only published products (from materialized view)
- US-018: draft and soft-deleted products are NEVER returned
- US-018: pagination cursor works correctly
- US-018: locale header resolves i18n name field
- US-018: ?locale= override takes precedence over Accept-Language
- US-019: GET /api/search?q= returns matching published products
- US-019: search respects all visibility conditions (same as product listing)
- US-019: short queries (< 2 chars) return 400
"""
...
async def test_product_list_resolves_name_to_requested_locale(client: Any, seeded_db: None) -> None:
    """US-018: Accept-Language: ar returns Arabic product names."""
...
async def test_product_list_locale_param_overrides_accept_language(
    ...
    """US-018: ?locale=tr overrides Accept-Language: ar."""
...
async def test_product_list_cursor_pagination_works(client: Any, seeded_db: None) -> None:
    """US-018: cursor pagination returns distinct pages without overlap."""
...
async def test_product_detail_returns_200_for_published_product(
    ...
    """US-018: GET /api/products/{slug} returns product detail."""
...
async def test_product_detail_returns_404_for_draft(client: Any, seeded_db: None) -> None:
    """US-018: draft products return 404 from the storefront."""
...
async def test_search_returns_matching_published_products(client: Any, seeded_db: None) -> None:
    """US-019: GET /api/search?q= returns matching products from materialized view."""
...
async def test_search_excludes_draft_products(client: Any, seeded_db: None) -> None:
    """US-019: search results never include draft products."""
...
async def test_search_short_query_returns_400(client: Any) -> None:
    """US-019: query shorter than 2 characters returns 400."""
...
async def test_search_arabic_term_finds_arabic_product(client: Any, seeded_db: None) -> None:
    """US-019: search across all locale values in the tsvector."""
