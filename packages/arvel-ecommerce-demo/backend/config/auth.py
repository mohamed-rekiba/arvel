...
``providers.users.model`` points to the demo's ``User`` model (integer PKs).
``providers.users.resource`` points to the demo's ``UserResource`` which
...
jwt: dict[str, object] = {
    "secret": env("APP_KEY", ""),
    "algorithm": "HS256",
    "issuer": env("JWT_ISSUER", "arvel-demo"),
    "audience": env("JWT_AUDIENCE", "arvel-demo"),
...
routes: dict[str, object] = {
    # Disabled — the demo registers its own /api/auth/login that accepts
