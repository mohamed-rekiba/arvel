...
<template>
  <div class="flex gap-4 overflow-x-auto pb-2 scrollbar-hide sm:gap-6">
    <RouterLink
    ...
    >
      <div
        class="flex h-20 w-20 items-center justify-center rounded-full border-2 border-transparent bg-primary-50 shadow-sm transition duration-200 group-hover:border-brand group-hover:bg-primary-100 group-hover:shadow-md"
      >
        <span
      ...
      <span
        class="max-w-[80px] text-center text-xs font-medium text-fg-muted transition group-hover:text-brand"
      >
        {{ category.name }}
      </span>
    </RouterLink>
