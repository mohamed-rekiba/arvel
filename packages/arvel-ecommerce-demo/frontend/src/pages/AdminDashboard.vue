...
const { data, isPending } = useApiAdminOrdersIndexApiAdminOrdersGet()
const orders = computed(() => data.value?.data ?? [])

// KPIs
const totalRevenue = computed(() => orders.value.reduce((sum, o) => sum + o.total, 0))
const totalOrders = computed(() => orders.value.length)
const uniqueCustomers = computed(() => new Set(orders.value.map((o) => o.user?.id)).size)
...
<template>
  <div class="space-y-6">
    ...
    <template v-else>
      ...
      <div class="grid gap-6 lg:grid-cols-2">
        <RecentOrdersCard :orders="orders.slice(0, 6)" :locale="currentLocale" />
