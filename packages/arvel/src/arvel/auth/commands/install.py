"""``auth:install`` — publish all auth package files into the consumer app."""
...
_AUTH_TAGS = (
    ...
    "arvel-auth-views",
    "arvel-auth-routes",
    "arvel-auth-migrations",
)


class AuthInstallCommand(Command):
    """Publish auth config, views, routes, and migrations into the application."""
    ...
    def install(self, *, force: bool) -> int:
        if self.app is None:
            typer.echo("arvel: auth:install requires a framework Application", err=True)
            return 2
        ...
        items = [i for i in all_items if i.tag in _AUTH_TAGS]

        if not items:
            typer.echo("Nothing to publish (no auth tags registered).")
            return 0
        ...
        try:
            ...
            typer.echo(f"arvel: auth:install failed: {exc}", err=True)
            return 1
