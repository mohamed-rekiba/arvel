...
def test_where_full_text_emits_at_at_operator() -> None:
    ...
    sql = _sql(FtsPost.where_full_text(FtsPost.search_vector, "python async"))
    assert "@@" in sql
...
def test_where_full_text_websearch_to_tsquery() -> None:
    ...
    sql = _sql(
        FtsPost.where_full_text(
            FtsPost.search_vector, "python -boring", tsquery_fn="websearch_to_tsquery"
        )
    )
...
def test_where_full_text_to_tsquery() -> None:
    ...
    sql = _sql(
        FtsPost.where_full_text(FtsPost.search_vector, "python & async", tsquery_fn="to_tsquery")
    )
...
def test_where_full_text_phraseto_tsquery() -> None:
    ...
    sql = _sql(
        FtsPost.where_full_text(
            FtsPost.search_vector, "python asyncio", tsquery_fn="phraseto_tsquery"
        )
    )
...
def test_where_full_text_invalid_tsquery_fn_raises() -> None:
    ...
    with pytest.raises(ValueError, match="tsquery_fn"):
        FtsPost.where_full_text(FtsPost.search_vector, "python", tsquery_fn="exec('rm -rf')")


def test_where_full_text_custom_lang() -> None:
    """The lang parameter is embedded in the compiled SQL."""
    sql = _sql(FtsPost.where_full_text(FtsPost.search_vector, "python", lang="french"))
...
def test_where_full_text_query_is_bind_param() -> None:
    ...
    sql = _sql(FtsPost.where_full_text(FtsPost.search_vector, "DROP TABLE--"))
...
def test_where_full_text_chains_with_where() -> None:
    ...
    sql = _sql(FtsPost.where_full_text(FtsPost.search_vector, "python").where(FtsPost.id > 10))
    assert "@@" in sql
...
def test_where_full_text_on_query_builder_directly() -> None:
    ...
    sql = (
        FtsPost.query()
        .where_full_text(FtsPost.search_vector, "python")
...
def test_where_full_text_returns_self_for_chaining() -> None:
    ...
    qb = FtsPost.where_full_text(FtsPost.search_vector, "python")
...
def test_order_by_relevance_emits_ts_rank() -> None:
    ...
    sql = _sql(FtsPost.order_by_relevance(FtsPost.search_vector, "python"))
...
def test_order_by_relevance_custom_lang() -> None:
    ...
    sql = _sql(FtsPost.order_by_relevance(FtsPost.search_vector, "python", lang="spanish"))
...
def test_order_by_relevance_query_is_bind_param() -> None:
    ...
    sql = _sql(FtsPost.order_by_relevance(FtsPost.search_vector, "safe search"))
    assert "ts_rank" in sql
...
def test_order_by_relevance_stacks_with_other_order_by() -> None:
    ...
    sql = _sql(FtsPost.order_by_relevance(FtsPost.search_vector, "python").order_by(FtsPost.id))
    assert "ts_rank" in sql
...
def test_order_by_relevance_with_where_full_text() -> None:
    ...
    sql = _sql(
        FtsPost.where_full_text(FtsPost.search_vector, "python async").order_by_relevance(
            FtsPost.search_vector, "python async"
        )
    )
    assert "@@" in sql
    assert "ts_rank" in sql
...
def test_order_by_relevance_on_query_builder_directly() -> None:
    ...
    sql = (
        FtsPost.query()
        .order_by_relevance(FtsPost.search_vector, "python")
        .to_sql(dialect="postgresql")
    )
    assert "ts_rank" in sql
...
def test_order_by_relevance_returns_self_for_chaining() -> None:
    ...
    qb = FtsPost.order_by_relevance(FtsPost.search_vector, "python")
    assert isinstance(qb, QueryBuilder)
