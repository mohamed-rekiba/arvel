...
async def test_upsert_single_statement_returns_count(
    ...
    x = await WTag.where(WTag.__table__.c.slug == "x").first()
...
async def test_increment_each_bumps_multiple(engine: AsyncEngine, session: AsyncSession) -> None:
    ...
    affected = await WCounter.where(WCounter.__table__.c.name == "c").increment_each(
        {"hits": 5, "misses": 2}
    )
    assert affected == 1

    c = await WCounter.where(WCounter.__table__.c.name == "c").first()
...
async def test_decrement_each_bumps_multiple(engine: AsyncEngine, session: AsyncSession) -> None:
    ...
    await WCounter.where(WCounter.__table__.c.name == "d").decrement_each({"hits": 3, "misses": 1})

    d = await WCounter.where(WCounter.__table__.c.name == "d").first()
