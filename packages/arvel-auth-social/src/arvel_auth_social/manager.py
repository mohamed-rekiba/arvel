"""SocialAuthManager — resolves configured providers by name."""
...
class SocialAuthManager:
    """Builds provider instances from a :class:`SocialAuthConfig`."""
    ...
    def provider(self, name: str) -> OAuthProvider:
        ...
        if builder is None:
            raise SocialProviderNotFound(name, self.configured_providers())
        return builder()
    ...
    async def oidc(self) -> OIDCProvider:
        ...
        if not (c.oidc_issuer_url and c.oidc_client_id):
            raise SocialProviderNotFound("oidc", self.configured_providers())
