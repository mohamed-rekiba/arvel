"""SocialAccountLinker — user resolution, link reuse, and duplicate protection."""
...
async def test_first_login_creates_user_and_account(async_session: AsyncSession) -> None:
    account = await SocialAccountLinker(async_session).link(_oauth_user(), _TOKEN)
...
async def test_links_to_existing_verified_email(async_session: AsyncSession) -> None:
    ...
    account = await SocialAccountLinker(async_session).link(
...
async def test_repeat_login_updates_existing_account(async_session: AsyncSession) -> None:
    linker = SocialAccountLinker(async_session)
    ...
    rows = (await async_session.execute(select(SocialAccount))).scalars().all()
    assert len(rows) == 1
...
async def test_unique_constraint_blocks_duplicate_link(async_session: AsyncSession) -> None:
    ...
    await SocialAccountLinker(async_session).link(_oauth_user(), _TOKEN)
    ...
    await async_session.flush()
    async_session.add(SocialAccount(user_id=other.id, provider="google", provider_id="g-1"))
    with pytest.raises(IntegrityError):
        await async_session.flush()


def test_duplicate_exception_message() -> None:
    exc = DuplicateSocialAccount("google", "g-1")
