"""Service-container parity: extend-after-resolve, after_resolving hook, bind_method,
resolving-fires-for-scoped."""
...
# --- extend() after a singleton is resolved -----------------------------
def test_extend_after_resolved_singleton_transforms_cached_instance() -> None:
    c = Container()
...
def test_extend_before_resolve_applies_at_build() -> None:
    c = Container()
    c.singleton("box", lambda app: Box(1))
...
# --- after_resolving ----------------------------------------------------
def test_after_resolving_fires_after_resolving() -> None:
    c = Container()
    order: list[str] = []
...
# --- bind_method --------------------------------------------------------
def test_bind_method_overrides_call() -> None:
    class Job:
        def handle(self) -> str:
            return "real"
...
def test_unbound_method_resolves_normally() -> None:
    class Job:
        def handle(self) -> str:
            return "real"
...
# --- resolving fires for scoped builds ----------------------------------
def test_resolving_fires_for_scoped_build() -> None:
    c = Container()
    seen: list[Any] = []
