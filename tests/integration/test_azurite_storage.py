...
async def test_azure_temporary_url_is_a_signed_sas_url(
    azurite_conn: str, configure_app: Any
) -> None:
    ...
    disk = FilesystemManager(app).disk("azure")
    await to_thread.run_sync(disk.fs.mkdir, container)

    await disk.put("docs/report.pdf", b"pdf bytes")
    url = await disk.temporary_url("docs/report.pdf", timedelta(minutes=5))
    assert "docs/report.pdf" in url
    assert "sig=" in url  # a SAS signature — a genuinely signed, time-boxed URL, not the raw path
