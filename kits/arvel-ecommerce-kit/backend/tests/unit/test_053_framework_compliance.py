...
These tests verify that the e-commerce kit follows the Arvel framework's
documented patterns:
...
def test_no_route_decorators_in_storefront_controller() -> None:
    ...
    assert "@Route." not in src, (
        "@Route.* found in controllers/storefront.py — routes belong in routes/api.py"
    )
...
def test_no_route_decorators_in_auth_controller() -> None:
    ...
    assert "@Route." not in src, (
        "@Route.* found in controllers/auth.py — routes belong in routes/api.py"
    )
...
def test_admin_products_controller_inherits_controller() -> None:
    ...
    src = _src(ADMIN_CONTROLLERS_DIR / "products.py")
    assert "Controller" in src, "admin/products.py does not import or extend Controller"
    assert (
        "class AdminProductsController(Controller)" in src
        or "class ProductsController(Controller)" in src
    ), "admin products controller class does not extend Controller"
...
def test_admin_categories_controller_inherits_controller() -> None:
    ...
    ), "admin categories controller class does not extend Controller"
...
def test_admin_vendors_controller_inherits_controller() -> None:
    ...
    ), "admin vendors controller class does not extend Controller"
...
def test_admin_orders_controller_inherits_controller() -> None:
    ...
    ), "admin orders controller class does not extend Controller"
...
def test_storefront_controller_inherits_controller() -> None:
    ...
    ), "StorefrontController does not extend Controller"
...
def test_routes_api_uses_route_group() -> None:
    ...
    assert "Route.group(" in src, "Route.group() not found in routes/api.py"
...
def test_routes_api_uses_framework_routing() -> None:
    ...
    assert "Route." in src, "Route facade not used in routes/api.py"
...
def test_routes_api_registers_products_routes() -> None:
    ...
    assert "AdminProductsController" in src, (
        "AdminProductsController not referenced in routes/api.py"
    )
    assert '"/products"' in src or "products" in src, (
        "/products routes not declared in routes/api.py"
    )
...
def test_routes_api_registers_categories_routes() -> None:
    ...
    assert "AdminCategoriesController" in src, (
        "AdminCategoriesController not referenced in routes/api.py"
    )


def test_routes_api_registers_vendors_routes() -> None:
    src = _src(ROUTES_FILE)
    assert "AdminVendorsController" in src, "AdminVendorsController not referenced in routes/api.py"
...
def test_category_service_exists() -> None:
    ...
    assert service_file.exists(), "app/services/category_service.py does not exist"
...
def test_category_service_has_list_method() -> None:
    ...
    assert "async def list" in src or "def list" in src, "CategoryService has no list method"
...
def test_category_service_has_find_method() -> None:
    ...
    if not service_file.exists():
        return
    src = service_file.read_text()
    assert "async def find" in src or "def find" in src, "CategoryService has no find method"
...
def test_category_service_has_to_dict_method() -> None:
    ...
    if not service_file.exists():
        return
    src = service_file.read_text()
    assert "def to_dict" in src, "CategoryService has no to_dict serialization method"


def test_vendor_service_exists() -> None:
    """app/services/vendor_service.py must exist."""
    service_file = SERVICES_DIR / "vendor_service.py"
    assert service_file.exists(), "app/services/vendor_service.py does not exist"
...
def test_vendor_service_has_list_method() -> None:
    ...
    assert "async def list" in src or "def list" in src, "VendorService has no list method"
...
def test_vendor_service_has_to_dict_method() -> None:
    ...
    if not service_file.exists():
        return
    src = service_file.read_text()
    assert "def to_dict" in src, "VendorService has no to_dict serialization method"


# ─── JsonResource subclasses ─────────────────────────────────────────


def test_json_resource_used_in_kit() -> None:
    ...
    for py_file in (CONTROLLERS_DIR / "..").rglob("*.py"):
        ...
        if "JsonResource" in py_file.read_text():
            ...
            break
    assert found, (
        "JsonResource not used anywhere in app/http/ — "
...
def test_all_original_endpoints_still_declared_in_routes() -> None:
    ...
    for ctrl in [
        ...
        assert ctrl in src, f"{ctrl} not found in routes/api.py"
    ...
    for snippet in ["StorefrontController", "CartController", "CheckoutController"]:
        assert snippet in src, f"{snippet} not found in routes/api.py"
