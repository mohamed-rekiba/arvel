...
export const useCartItemsStoreApiCartItemsPost = <TError = HTTPValidationError, TContext = unknown>(
  ...
  TContext
> => {
  return useMutation(getCartItemsStoreApiCartItemsPostMutationOptions(options), queryClient)
}
/**
...
>(
  ...
  TContext
> => {
  return useMutation(getCartItemsUpdateApiCartItemsItemIdPatchMutationOptions(options), queryClient)
}
/**
...
export const cartItemsDestroyApiCartItemsItemIdDelete = (
  ...
  return request<CartWrapperOut>(
    { url: `/api/cart/items/${itemId}`, method: 'DELETE', params: unref(params), signal },
    options,
  )
}
...
> => {
  return useMutation(
    getCartItemsDestroyApiCartItemsItemIdDeleteMutationOptions(options),
    queryClient,
  )
}
